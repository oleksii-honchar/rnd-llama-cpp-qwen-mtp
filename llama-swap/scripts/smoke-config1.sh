#!/usr/bin/env bash
# Smoke: config-1 — Qwen3.6-35B-A3B-TQ3_4S (TurboQuant TQ3_4S) chat model.
#
...
#
# Usage (from llama-swap):
#   ./scripts/smoke-config1.sh
# Optional: LLAMA_SWAP_URL=http://host:8014 ./scripts/smoke-config1.sh
#
# Prereq: stack running (e.g. ./start.sh swap.config1).
set -euo pipefail
...
# First load of 35B-A3B TQ model can exceed 180s on cold GPU
...
# Must match configs/config-1.yaml: keys under `models:` (GET /v1/models lists primaries; aliases optional).
MODEL_IDS_IN_CATALOG=(
  qwen36-35b-tq3
)

# Primaries + every `aliases:` entry from config-1.
MODEL_IDS_CHAT=(
  qwen36-35b-tq3
  qwen36-35b-tq3-precise
)

command -v jq >/dev/null 2>&1 || { echo "smoke-config1.sh: jq required" >&2; exit 1; }
...
echo "=== smoke mammoth config-1 (${LLAMA_SWAP_URL}) ==="
